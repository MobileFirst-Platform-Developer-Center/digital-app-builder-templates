...
export class List extends React.Component {
 ...
 renderList(){
  ...
  this.itemList.map((item, index) => {
      renderList.push(
        <div key={index} style={{ display: 'flex',flexDirection: 'column', justifyContent: 'center', backgroundColor: '#091b40', width: '50%', marginBottom: '2%'}}>
          <img src={item.image} alt="avtar" width="100%"></img>
          <div style={{ marginLeft: '2%'}}>
          <h2 style={{ color: "#ffffff"}}>{item.heading}</h2>
          <h3 style={{ color: "#ffffff"}}>{item.subHeading}</h3>
          <h5 style={{ color: "#ffffff"}}>{item.description}</h5>
          </div>
        </div>
    )
